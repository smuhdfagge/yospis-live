    ...
    <div class="max-w-4xl">
        ...
        <div class="bg-white rounded-xl shadow-sm">
            <div class="p-6 border-b border-gray-200">
                ...
                <p class="text-sm text-gray-500 mt-1">Update partner details</p>
            </div>
            ...
            <form action="{{ route('admin.partners.update', $partner) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="type" id="type" required
                            ...
                            <option value="donor" {{ old('type', $partner->type) == 'donor' ? 'selected' : '' }}>Donor/Funder</option>
                            <option value="implementing" {{ old('type', $partner->type) == 'implementing' ? 'selected' : '' }}>Implementing Partner</option>
                            <option value="government" {{ old('type', $partner->type) == 'government' ? 'selected' : '' }}>Government Agency</option>
                            <option value="media" {{ old('type', $partner->type) == 'media' ? 'selected' : '' }}>Media Partner</option>
                            <option value="corporate" {{ old('type', $partner->type) == 'corporate' ? 'selected' : '' }}>Corporate Partner</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    @if($partner->logo)
                        <div class="mb-4">
                            <img src="{{ Storage::url($partner->logo) }}" alt="{{ $partner->name }}" class="w-24 h-24 object-contain bg-gray-50 rounded-lg">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Leave empty to keep current logo. Max 1MB.</p>
                    @error('logo')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $partner->sort_order) }}" min="0"
